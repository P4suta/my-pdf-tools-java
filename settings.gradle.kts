...
// The ninth shared module: :shared:progress — the cross-app progress/lifecycle event island. It
// owns the SINGLE vocabulary of events a pdfbook conversion emits (ProgressEvent), the ProgressSink
// an emitter writes them to, and the one-object-per-line JSONL wire codec (JsonlProgressCodec) that
// carries them across a process boundary: a pdfbook subprocess writes them to its --progress-file,
// the :webapp driving adapter reads them back to drive an SSE stream. Pure framework-free value
// types + a dependency-free codec; leans on :shared:kernel for the precondition checks only. Gradle
// maps :shared:progress -> shared/progress/ automatically.
include(":shared:progress")

// The unified web feature: :webapp is a top-level driving adapter over the pdfbook pipeline, peer to
// the four processing features above, with the standard five hexagon layers. It is NOT generated by
// the forEach (its :app is a Spring Boot front end, not a Commons CLI one) and it runs pdfbook
// out-of-process — :webapp:infrastructure shells out to the packaged pdfbook binary as an external
// tool, so the feature has ZERO compile dependency on :pipeline:* / :tate-yoko-pdf:*. Spring lives
// only in :webapp:app. Layers are added here as they are built; Gradle maps :webapp:<layer> ->
// webapp/<layer>/ automatically.
