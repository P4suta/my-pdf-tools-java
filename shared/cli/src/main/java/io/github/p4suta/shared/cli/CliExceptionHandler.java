 ...
 *
 * <p>The kind, exit code, masked message, and (verbose) technical detail all come from the shared
 * {@link ExceptionMapper}, which reads the exit code and severity off the resolved {@link
 * ErrorCategory} and masks absolute paths via {@code PiiSanitizer}. An app may pass an extra {@code
 * throwable -> ErrorCategory} rule (e.g. an {@code UncheckedIOException} unwrap) that the mapper
 * consults before its shared baseline.
 */
public final class CliExceptionHandler {
    ...
    public int handle(Throwable ex) {
        ...
        err.println("Error[" + mapping.kind().name() + "]: " + mapping.safeUserMessage());
